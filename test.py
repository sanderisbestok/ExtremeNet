...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join("/home/hansen", "results", "test", "extremenet"), exist_ok = True)
    ...
    for image_id, image_name in enumerate(image_names):
        ...
        for cls_ind in top_bboxes[image_id]:
            for bbox in top_bboxes[image_id][cls_ind]:
                ...
                bbox[2] -= bbox[0]
                bbox[3] -= bbox[1]
                bbox_out  = list(map(float, bbox[0:4]))
                bbox_out.insert(0, float("{:.2f}".format(bbox[4])))
                bbox_out.insert(0, cls_ind)
                bboxes.append(bbox_out)

        with open(os.path.join("/home/hansen", "results", "test", "extremenet", os.path.basename(image_name)[:-4]+".txt"),"w+") as my_csv:
